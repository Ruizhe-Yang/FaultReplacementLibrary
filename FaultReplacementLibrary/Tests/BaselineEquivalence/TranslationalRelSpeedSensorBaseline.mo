...
model TranslationalRelSpeedSensorBaseline "Executable mechanical sensor Normal equivalence test"
  ...
  Modelica.Mechanics.Translational.Sensors.RelSpeedSensor original annotation(Placement(transformation(extent={{30,-50},{50,-30}})));
  FaultReplacementLibrary.Mechanics.Translational.Sensors.FaultableRelSpeedSensor faultable(severity=0) annotation(Placement(transformation(extent={{70,30},{90,50}})));
equation
  ...
  assert(noEvent(abs(original.v_rel-faultable.v_rel)<1e-7),"TranslationalRelSpeedSensor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TranslationalRelSpeedSensorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TranslationalRelSpeedSensorBaseline;
