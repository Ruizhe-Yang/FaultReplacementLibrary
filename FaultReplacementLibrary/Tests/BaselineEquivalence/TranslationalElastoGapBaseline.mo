...
model TranslationalElastoGapBaseline "MSL ElastoGap and faultable annotation(Placement(transformation(extent={{70,-50},{90,-30}}))) Normal equivalence"
  Modelica.Mechanics.Translational.Components.ElastoGap original(c=100,d=20,s_rel0=0.01) annotation(Placement(transformation(extent={{38,30},{58,50}})));
  FaultReplacementLibrary.Mechanics.Translational.Components.FaultableElastoGap faultable(c=100,d=20,s_rel0=0.01,severity=0);
...
equation
  ...
  assert(noEvent(abs(original.f-faultable.f)<1e-6),"Translational ElastoGap Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TranslationalElastoGapBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TranslationalElastoGapBaseline;
