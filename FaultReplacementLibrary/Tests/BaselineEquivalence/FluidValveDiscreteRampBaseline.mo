...
model FluidValveDiscreteRampBaseline "Executable Modelica.Fluid component Normal equivalence test"
  replaceable package Medium=Modelica.Media.Water.StandardWater;
  inner Modelica.Fluid.System system;
  Modelica.Blocks.Sources.BooleanConstant command(k=true) annotation(Placement(transformation(extent={{-90,30},{-70,50}})));
  ...
  Modelica.Fluid.Valves.ValveDiscreteRamp original(redeclare package Medium=Medium,dp_nominal=1e5,m_flow_nominal=1,Topen=0.1,Tclose=0.1) annotation(Placement(transformation(extent={{-50,-50},{-30,-30}})));
  FaultReplacementLibrary.Fluid.Valves.FaultableValveDiscreteRamp faultable(redeclare package Medium=Medium,dp_nominal=1e5,m_flow_nominal=1,Topen=0.1,Tclose=0.1,severity=0) annotation(Placement(transformation(extent={{-10,30},{10,50}})));
equation
  connect(command.y,original.open)
    annotation(Line(points={{-70,40},{-55,40},{-55,-40},{-40,-40}}, color={0,0,127})); connect(command.y,faultable.open)
  ...
  assert(noEvent(abs(original.port_a.m_flow-faultable.port_a.m_flow)<1e-5),"FluidValveDiscreteRamp Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 FluidValveDiscreteRampBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-7));
end FluidValveDiscreteRampBaseline;
