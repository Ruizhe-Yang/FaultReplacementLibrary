...
model SignalVoltageBaseline "Executable source Normal/severity=0 equivalence test"
  Modelica.Blocks.Sources.Sine command(amplitude=2,f=1,offset=0.2) annotation(Placement(transformation(extent={{-90,30},{-70,50}})));
  Modelica.Electrical.Analog.Sources.SignalVoltage original annotation(Placement(transformation(extent={{-50,-50},{-30,-30}})));
  FaultReplacementLibrary.Electrical.Analog.Sources.FaultableSignalVoltage faultable(severity=0) annotation(Placement(transformation(extent={{-10,30},{10,50}})));
...
equation
  connect(command.y,original.v)
    annotation(Line(points={{-70,40},{-55,40},{-55,-40},{-40,-40}}, color={0,0,127})); connect(command.y,faultable.v)
  ...
  assert(noEvent(abs(loadOriginal.i-loadFaultable.i)<1e-8),"SignalVoltage Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 SignalVoltageBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end SignalVoltageBaseline;
