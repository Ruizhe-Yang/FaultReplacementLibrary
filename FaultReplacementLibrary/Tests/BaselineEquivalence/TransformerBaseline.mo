...
model TransformerBaseline "MSL Transformer and FaultableTransformer Normal equivalence"
  Modelica.Electrical.Analog.Basic.Transformer original(L1=1,L2=1,M=0.8) annotation(Placement(transformation(extent={{-50,30},{-30,50}})));
  FaultReplacementLibrary.Electrical.Analog.Basic.FaultableTransformer faultable(L1=1,L2=1,M=0.8,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  connect(faultable.p2,loadFaultable.p)
    ...
    annotation(Line(points={{90,-40},{45,-40},{45,-30},{0,-30}}, color={0,0,255}));
  assert(noEvent(abs(original.i1-faultable.i1)<1e-6 and abs(original.i2-faultable.i2)<1e-6),"Transformer Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TransformerBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TransformerBaseline;
