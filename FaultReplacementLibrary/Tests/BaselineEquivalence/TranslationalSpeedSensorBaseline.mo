...
model TranslationalSpeedSensorBaseline "Executable mechanical sensor Normal equivalence test"
  ...
  Modelica.Mechanics.Translational.Sensors.SpeedSensor original annotation(Placement(transformation(extent={{17,-50},{37,-30}})));
  FaultReplacementLibrary.Mechanics.Translational.Sensors.FaultableSpeedSensor faultable(severity=0) annotation(Placement(transformation(extent={{70,30},{90,50}})));
equation
  ...
  assert(noEvent(abs(original.v-faultable.v)<1e-7),"TranslationalSpeedSensor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TranslationalSpeedSensorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TranslationalSpeedSensorBaseline;
