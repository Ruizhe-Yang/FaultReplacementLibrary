...
model FluidStaticPipeBaseline "Executable Modelica.Fluid component Normal equivalence test"
  ...
  Modelica.Fluid.Pipes.StaticPipe original(redeclare package Medium=Medium,length=1,diameter=0.02) annotation(Placement(transformation(extent={{-37,30},{-17,50}})));
  FaultReplacementLibrary.Fluid.Pipes.FaultableStaticPipe faultable(redeclare package Medium=Medium,length=1,diameter=0.02,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
equation
  ...
  assert(noEvent(abs(original.port_a.m_flow-faultable.port_a.m_flow)<1e-5),"FluidStaticPipe Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 FluidStaticPipeBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-7));
end FluidStaticPipeBaseline;
