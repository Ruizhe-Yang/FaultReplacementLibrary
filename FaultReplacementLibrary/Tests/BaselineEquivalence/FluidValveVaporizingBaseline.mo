...
model FluidValveVaporizingBaseline "Executable Modelica.Fluid component Normal equivalence test"
  ...
  Modelica.Fluid.Valves.ValveVaporizing original(
    redeclare package Medium=Medium,
    CvData=Modelica.Fluid.Types.CvTypes.Av,
    Av=1e-4,
    dp_nominal=1e5,
    m_flow_nominal=1) annotation(Placement(transformation(extent={{-50,-50},{-30,-30}})));
  FaultReplacementLibrary.Fluid.Valves.FaultableValveVaporizing faultable(
    redeclare package Medium=Medium,
    CvData=Modelica.Fluid.Types.CvTypes.Av,
    Av=1e-4,
    dp_nominal=1e5,
    m_flow_nominal=1,
    severity=0) annotation(Placement(transformation(extent={{-10,30},{10,50}})));
equation
  ...
  assert(noEvent(abs(original.port_a.m_flow-faultable.port_a.m_flow)<1e-5),"FluidValveVaporizing Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 FluidValveVaporizingBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-7));
end FluidValveVaporizingBaseline;
