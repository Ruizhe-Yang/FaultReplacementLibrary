...
model MagneticLeakageWithCoefficientBaseline "Executable magnetic Normal equivalence test"
  Modelica.Blocks.Sources.Constant control(k=1e5) annotation(Placement(transformation(extent={{-10,30},{10,50}})));
  ...
  Modelica.Magnetic.FluxTubes.Basic.LeakageWithCoefficient original(c_usefulFlux=0.7) annotation(Placement(transformation(extent={{30,-50},{50,-30}})));
  FaultReplacementLibrary.Magnetic.FluxTubes.Basic.FaultableLeakageWithCoefficient faultable(c_usefulFlux=0.7,severity=0) annotation(Placement(transformation(extent={{70,30},{90,50}})));
...
equation
  ...
  connect(control.y,original.R_mUsefulTot)
    annotation(Line(points={{10,40},{25,40},{25,-40},{40,-40}}, color={0,0,127})); connect(control.y,faultable.R_mUsefulTot)
  ...
  assert(noEvent(abs(sourceOriginal.Phi-sourceFaultable.Phi)<1e-10),"Magnetic LeakageWithCoefficient Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 MagneticLeakageWithCoefficientBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end MagneticLeakageWithCoefficientBaseline;
