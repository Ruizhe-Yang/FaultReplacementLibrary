...
model RotationalSpringDamperBaseline "MSL rotational SpringDamper and faultable annotation(Placement(transformation(extent={{70,-50},{90,-30}}))) Normal equivalence"
  Modelica.Mechanics.Rotational.Components.SpringDamper original(c=100,d=2,phi_rel0=0.1) annotation(Placement(transformation(extent={{38,30},{58,50}})));
  FaultReplacementLibrary.Mechanics.Rotational.Components.FaultableSpringDamper faultable(c=100,d=2,phi_rel0=0.1,severity=0);
  ...
  Modelica.Blocks.Sources.Ramp leftAngle(height=1,duration=1) annotation(Placement(transformation(extent={{-90,30},{-70,50}}))); Modelica.Blocks.Sources.Constant rightAngle(k=0) annotation(Placement(transformation(extent={{-26,-50},{-6,-30}})));
equation
  ...
  assert(noEvent(abs(original.tau-faultable.tau)<1e-7),"Rotational SpringDamper Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 RotationalSpringDamperBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end RotationalSpringDamperBaseline;
