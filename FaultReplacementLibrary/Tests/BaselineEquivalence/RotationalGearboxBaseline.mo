...
model RotationalGearboxBaseline "MSL Gearbox and faultable annotation(Placement(transformation(extent={{70,30},{90,50}}))) Normal equivalence"
  Modelica.Mechanics.Rotational.Components.Gearbox original(ratio=2,c=100,d=2,b=0.01) annotation(Placement(transformation(extent={{-26,-50},{-6,-30}})));
  FaultReplacementLibrary.Mechanics.Rotational.Components.FaultableGearbox faultable(ratio=2,c=100,d=2,b=0.01,severity=0);
...
equation
  ...
  connect(driveFaultable.flange,faultable.flange_a)
    ...
    annotation(Line(points={{26,-40},{48,-40}}, color={0,0,0}));
  assert(noEvent(abs(original.flange_b.phi-faultable.flange_b.phi)<1e-6 and abs(original.flange_a.tau-faultable.flange_a.tau)<1e-5),"Rotational Gearbox Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 RotationalGearboxBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end RotationalGearboxBaseline;
