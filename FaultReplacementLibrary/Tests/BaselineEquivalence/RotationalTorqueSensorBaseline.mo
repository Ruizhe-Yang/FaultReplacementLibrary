...
model RotationalTorqueSensorBaseline "Executable mechanical sensor Normal equivalence test"
  ...
  Modelica.Mechanics.Rotational.Components.Spring loadOriginal(c=100) annotation(Placement(transformation(extent={{6,30},{26,50}}))),loadFaultable(c=100) annotation(Placement(transformation(extent={{6,-50},{26,-30}})));
  ...
  Modelica.Mechanics.Rotational.Sensors.TorqueSensor original annotation(Placement(transformation(extent={{-26,-50},{-6,-30}})));
  FaultReplacementLibrary.Mechanics.Rotational.Sensors.FaultableTorqueSensor faultable(severity=0) annotation(Placement(transformation(extent={{70,30},{90,50}})));
equation
  ...
  connect(driveFaultable.flange,faultable.flange_a)
    ...
    annotation(Line(points={{26,-40},{48,-40}}, color={0,0,0}));
  assert(noEvent(abs(original.tau-faultable.tau)<1e-7),"RotationalTorqueSensor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 RotationalTorqueSensorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end RotationalTorqueSensorBaseline;
