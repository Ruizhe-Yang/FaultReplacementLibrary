...
model MagneticPotentialDifferenceSensorBaseline "Executable magnetic potential sensor Normal equivalence test"
  ...
  Modelica.Magnetic.FluxTubes.Sensors.MagneticPotentialDifferenceSensor original annotation(Placement(transformation(extent={{17,30},{37,50}})));
  FaultReplacementLibrary.Magnetic.FluxTubes.Sensors.FaultableMagneticPotentialDifferenceSensor faultable(severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  assert(noEvent(abs(original.V_m-faultable.V_m)<1e-10),"MagneticPotentialDifferenceSensor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 MagneticPotentialDifferenceSensorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end MagneticPotentialDifferenceSensorBaseline;
