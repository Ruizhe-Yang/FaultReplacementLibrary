...
model VariableConductorBaseline "Executable VariableConductor Normal/severity=0 equivalence test"
  ...
  Modelica.Electrical.Analog.Basic.VariableConductor original annotation(Placement(transformation(extent={{-50,-50},{-30,-30}})));
  FaultReplacementLibrary.Electrical.Analog.Basic.FaultableVariableConductor faultable(severity=0) annotation(Placement(transformation(extent={{-10,30},{10,50}})));
...
equation
  connect(control.y,original.G)
    annotation(Line(points={{-70,40},{-55,40},{-55,-40},{-40,-40}}, color={0,0,127})); connect(control.y,faultable.G)
  ...
  assert(noEvent(abs(original.i-faultable.i)<1e-7),"VariableConductor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 VariableConductorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end VariableConductorBaseline;
