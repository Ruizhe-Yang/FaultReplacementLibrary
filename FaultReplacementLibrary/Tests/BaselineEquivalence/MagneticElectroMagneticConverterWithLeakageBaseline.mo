...
model MagneticElectroMagneticConverterWithLeakageBaseline "MSL converter with leakage inductance and faultable annotation(Placement(transformation(extent={{70,-50},{90,-30}}))) Normal equivalence"
  Modelica.Magnetic.FluxTubes.Basic.ElectroMagneticConverterWithLeakageInductance original(N=100,L=0.01,A=1e-5,mu_rel=1) annotation(Placement(transformation(extent={{-37,30},{-17,50}})));
  FaultReplacementLibrary.Magnetic.FluxTubes.Basic.FaultableElectroMagneticConverterWithLeakageInductance faultable(N=100,L=0.01,A=1e-5,mu_rel=1,severity=0);
...
equation
  ...
  assert(noEvent(abs(original.i-faultable.i)<1e-6 and abs(original.Phi-faultable.Phi)<1e-8),"ConverterWithLeakage Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 MagneticElectroMagneticConverterWithLeakageBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end MagneticElectroMagneticConverterWithLeakageBaseline;
