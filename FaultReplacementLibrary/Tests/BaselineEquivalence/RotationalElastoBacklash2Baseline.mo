...
model RotationalElastoBacklash2Baseline "MSL ElastoBacklash2 and faultable annotation(Placement(transformation(extent={{70,-50},{90,-30}}))) Normal equivalence"
  Modelica.Mechanics.Rotational.Components.ElastoBacklash2 original(c=100,d=2,b=0.02) annotation(Placement(transformation(extent={{38,30},{58,50}})));
  FaultReplacementLibrary.Mechanics.Rotational.Components.FaultableElastoBacklash2 faultable(c=100,d=2,b=0.02,severity=0);
...
equation
  ...
  assert(noEvent(abs(original.tau-faultable.tau)<1e-6),"Rotational ElastoBacklash2 Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 RotationalElastoBacklash2Baseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end RotationalElastoBacklash2Baseline;
