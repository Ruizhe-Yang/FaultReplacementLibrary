...
model TranslationalRodBaseline "MSL Rod and faultable annotation(Placement(transformation(extent={{70,30},{90,50}}))) Normal equivalence"
  Modelica.Mechanics.Translational.Components.Rod original(L=0.3) annotation(Placement(transformation(extent={{-26,-50},{-6,-30}})));
  FaultReplacementLibrary.Mechanics.Translational.Components.FaultableRod faultable(L=0.3,severity=0);
...
equation
  connect(position.y,driveOriginal.s_ref)
    annotation(Line(points={{-70,40},{-48,40}}, color={0,0,127})); connect(position.y,driveFaultable.s_ref)
  ...
  connect(driveFaultable.flange,faultable.flange_a)
    ...
    annotation(Line(points={{26,-40},{48,-40}}, color={0,127,0}));
  assert(noEvent(abs(original.flange_a.f-faultable.flange_a.f)<1e-7 and abs(original.flange_b.s-faultable.flange_b.s)<1e-8),"Translational Rod Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TranslationalRodBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TranslationalRodBaseline;
