...
model CosineVoltageBaseline "Executable source Normal/severity=0 equivalence test"
  Modelica.Electrical.Analog.Sources.CosineVoltage original(V=2,f=1) annotation(Placement(transformation(extent={{-90,30},{-70,50}})));
  FaultReplacementLibrary.Electrical.Analog.Sources.FaultableCosineVoltage faultable(V=2,f=1,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  assert(noEvent(abs(loadOriginal.i-loadFaultable.i)<1e-8),"CosineVoltage Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 CosineVoltageBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end CosineVoltageBaseline;
