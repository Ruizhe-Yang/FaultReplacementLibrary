...
model ControlledIdealClosingSwitchBaseline "MSL controlled closing switch and faultable annotation(Placement(transformation(extent={{70,-50},{90,-30}}))) Normal equivalence"
  Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch original(level=0.5) annotation(Placement(transformation(extent={{-58,30},{-38,50}})));
  FaultReplacementLibrary.Electrical.Analog.Ideal.FaultableControlledIdealClosingSwitch faultable(level=0.5,severity=0);
...
equation
  ...
  assert(noEvent(abs(original.i-faultable.i)<1e-8),"ControlledIdealClosingSwitch Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 ControlledIdealClosingSwitchBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end ControlledIdealClosingSwitchBaseline;
