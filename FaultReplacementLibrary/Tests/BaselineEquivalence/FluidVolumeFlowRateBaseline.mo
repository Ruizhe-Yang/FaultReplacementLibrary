...
model FluidVolumeFlowRateBaseline "Executable Modelica.Fluid flow sensor Normal equivalence test"
  ...
  Modelica.Fluid.Sensors.VolumeFlowRate original(redeclare package Medium=Medium) annotation(Placement(transformation(extent={{-50,30},{-30,50}})));
  FaultReplacementLibrary.Fluid.Sensors.FaultableVolumeFlowRate faultable(redeclare package Medium=Medium,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  connect(sourceFaultable.ports[1],faultable.port_a)
    ...
    annotation(Line(points={{10,-40},{40,-40}}, color={0,127,255}));
  assert(noEvent(abs(original.V_flow-faultable.V_flow)<1e-6),"VolumeFlowRate Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 FluidVolumeFlowRateBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-7));
end FluidVolumeFlowRateBaseline;
