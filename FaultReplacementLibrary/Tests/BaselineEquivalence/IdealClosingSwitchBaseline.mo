...
model IdealClosingSwitchBaseline "MSL IdealClosingSwitch and faultable annotation(Placement(transformation(extent={{-26,30},{-6,50}}))) Normal equivalence"
  Modelica.Electrical.Analog.Ideal.IdealClosingSwitch original annotation(Placement(transformation(extent={{-58,-50},{-38,-30}})));
  FaultReplacementLibrary.Electrical.Analog.Ideal.FaultableIdealClosingSwitch faultable(severity=0);
...
equation
  ...
  assert(noEvent(abs(original.i-faultable.i)<1e-8),"IdealClosingSwitch Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 IdealClosingSwitchBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end IdealClosingSwitchBaseline;
