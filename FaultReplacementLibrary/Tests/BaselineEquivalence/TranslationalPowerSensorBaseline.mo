...
model TranslationalPowerSensorBaseline "Executable mechanical sensor Normal equivalence test"
  ...
  Modelica.Mechanics.Translational.Components.Damper loadOriginal(d=2) annotation(Placement(transformation(extent={{6,30},{26,50}}))),loadFaultable(d=2) annotation(Placement(transformation(extent={{6,-50},{26,-30}})));
  ...
  Modelica.Mechanics.Translational.Sensors.PowerSensor original annotation(Placement(transformation(extent={{-26,-50},{-6,-30}})));
  FaultReplacementLibrary.Mechanics.Translational.Sensors.FaultablePowerSensor faultable(severity=0) annotation(Placement(transformation(extent={{70,30},{90,50}})));
equation
  ...
  connect(driveFaultable.flange,faultable.flange_a)
    ...
    annotation(Line(points={{26,-40},{48,-40}}, color={0,127,0}));
  assert(noEvent(abs(original.power-faultable.power)<1e-7),"TranslationalPowerSensor Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 TranslationalPowerSensorBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end TranslationalPowerSensorBaseline;
