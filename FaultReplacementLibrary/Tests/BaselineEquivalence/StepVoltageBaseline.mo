...
model StepVoltageBaseline "Executable source Normal/severity=0 equivalence test"
  Modelica.Electrical.Analog.Sources.StepVoltage original(V=2,startTime=0.2) annotation(Placement(transformation(extent={{-90,30},{-70,50}})));
  FaultReplacementLibrary.Electrical.Analog.Sources.FaultableStepVoltage faultable(V=2,startTime=0.2,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  assert(noEvent(abs(loadOriginal.i-loadFaultable.i)<1e-8),"StepVoltage Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 StepVoltageBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end StepVoltageBaseline;
