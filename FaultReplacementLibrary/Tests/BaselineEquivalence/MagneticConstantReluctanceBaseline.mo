...
model MagneticConstantReluctanceBaseline "Executable magnetic Normal equivalence test"
  ...
  Modelica.Magnetic.FluxTubes.Basic.ConstantReluctance original(R_m=1e5) annotation(Placement(transformation(extent={{17,30},{37,50}})));
  FaultReplacementLibrary.Magnetic.FluxTubes.Basic.FaultableConstantReluctance faultable(R_m=1e5,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  assert(noEvent(abs(sourceOriginal.Phi-sourceFaultable.Phi)<1e-10),"Magnetic ConstantReluctance Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 MagneticConstantReluctanceBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
    experiment(StopTime=1,Tolerance=1e-8));
end MagneticConstantReluctanceBaseline;
