...
model VCVBaseline "MSL VCV and FaultableVCV Normal equivalence"
  Modelica.Electrical.Analog.Basic.VCV original(gain=2) annotation(Placement(transformation(extent={{-50,30},{-30,50}})));
  FaultReplacementLibrary.Electrical.Analog.Basic.FaultableVCV faultable(gain=2,severity=0) annotation(Placement(transformation(extent={{70,-50},{90,-30}})));
...
equation
  ...
  assert(noEvent(abs(original.v2-faultable.v2)<1e-9 and abs(original.i2-faultable.i2)<1e-9),"VCV Normal baseline mismatch");
  annotation(
    Documentation(info="<html><p>用法：直接仿真 VCVBaseline。该测试把名义 MSL 元件与 severity=0 的故障增强元件置于相同激励下，并以断言检查关键物理量的一致性；断言通过即表示 Normal 基线等价。</p></html>"),
