...
model ReplaceableTranslationalDamperSystem "Replaceable MSL translational damper system"
  Modelica.Blocks.Sources.Ramp command(height=0.2,duration=2) annotation(Placement(transformation(extent={{-90,30},{-70,50}})));
  ...
  replaceable Modelica.Mechanics.Translational.Components.Damper device(d=100)
    constrainedby Modelica.Mechanics.Translational.Interfaces.PartialCompliant
...
equation
  ...
  annotation(Documentation(info="<html><p>用法：将 ReplaceableTranslationalDamperSystem 作为可替换系统基类，通过 extends 派生场景，并在修改项中使用 redeclare 替换 device。保持其余拓扑和连接不变后仿真，可比较名义元件与故障增强元件。</p></html>"));
end ReplaceableTranslationalDamperSystem;
