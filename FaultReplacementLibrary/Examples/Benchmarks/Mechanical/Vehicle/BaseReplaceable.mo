...
model BaseReplaceable "One-dimensional vehicle with driving resistances"
  extends Modelica.Icons.Example;
  import Modelica.Constants.g_n;
  parameter Modelica.Units.SI.Mass m=100 "Mass of vehicle";
  parameter Modelica.Units.SI.Length R=0.25 "Radius of wheel";
  parameter Modelica.Units.SI.Area A=1 "Cross section of vehicle";
  parameter Real Cd=0.5 "Drag resistance coefficient";
  parameter Modelica.Units.SI.Density rho=1.18 "Density of air";
  parameter Modelica.Units.SI.Velocity vWind=0 "Constant wind velocity";
  parameter Real Cr=0.01 "Rolling resistance coefficient";
  //Check nominal force
  parameter Real inclination=0 "Constant inclination = tan(angle)";
  parameter Modelica.Units.SI.Velocity vNom=25/3.5 "Nominal velocity";
  final parameter Modelica.Units.SI.Force fDrag=Cd*A*rho*(vNom - vWind)^2/2 "Drag resistance"
    annotation(Dialog(enable=false));
  final parameter Modelica.Units.SI.Angle alpha=atan(inclination) "Inclination angle"
    annotation(Dialog(enable=false));
  final parameter Modelica.Units.SI.Force fRoll=Cr*m*g_n*cos(alpha) "Roll resistance"
    annotation(Dialog(enable=false));
  final parameter Modelica.Units.SI.Force fGrav=m*g_n*sin(alpha) "Grav resistance"
  annotation(Dialog(enable=false));
  replaceable Modelica.Mechanics.Translational.Components.Vehicle vehicle(
    m=m,
    J=0,
    R=R,
    A=A,
    ...
    s(fixed=true),
    v(fixed=true))
    constrainedby Modelica.Mechanics.Translational.Components.Vehicle annotation (Placement(transformation(extent={{30,-10},{50,10}})));
  Modelica.Mechanics.Translational.Components.Vehicle vehicle1(
    m=m,
    J=0,
    R=R,
    A=A,
  ...
  Modelica.Mechanics.Translational.Components.Vehicle trailer(
    m=m,
    J=0,
    R=R,
    A=A,
...
equation
  ...
  annotation (experiment(StopTime=60, Interval=0.01), Documentation(info="<html><p>用法：本模型保留 MSL 4.0.0 官方 Example 的系统拓扑与图形，作为 replaceable 基准。通过 extends BaseReplaceable(redeclare ...) 创建故障场景，不需要修改其余连接。</p>
<p>
