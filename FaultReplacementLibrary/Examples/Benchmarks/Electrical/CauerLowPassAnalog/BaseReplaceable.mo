...
model BaseReplaceable "Cauer low pass filter with analog components"
  ...
  parameter Modelica.Units.SI.Capacitance c2=1/(1.704992^2*l1)
    "Filter coefficient c2";
  parameter Modelica.Units.SI.Capacitance c3=1.682 "Filter coefficient c3";
  parameter Modelica.Units.SI.Capacitance c4=1/(1.179945^2*l2)
...
Modelica.Electrical.Analog.Basic.Capacitor C2(C=c2)
  ...
  replaceable Modelica.Electrical.Analog.Basic.Capacitor C3(C=c3,v(start=0, fixed=true))
    constrainedby Modelica.Electrical.Analog.Basic.Capacitor annotation (Placement(transformation(
        origin={0,-20},
        extent={{-10,-10},{10,10}},
        rotation=270)));
...
</html>", info="<html><p>用法：本模型保留 MSL 4.0.0 官方 Example 的系统拓扑与图形，作为 replaceable 基准。通过 extends BaseReplaceable(redeclare ...) 创建故障场景，不需要修改其余连接。</p>
