...
model FaultableCosineVoltage "CosineVoltage 独立故障增强模型"
  ...
  parameter Modelica.Units.SI.Voltage V(start=1) "Amplitude of cosine wave";
...
equation
  ...
  nominalOutput = offset + (if time < startTime then 0 else V*cos(2*Modelica.Constants.pi*f*(1+faultActivation*(if faultMode==FaultMode.FrequencyDrift then frequencyFault-1 else 0))*(time-startTime)+phase));
  v = effectiveOutput;
  annotation (
...
<li>Initially implemented by Christian Kral on 2013-05-14</li>
        ...
        info="<html><p>用法：将 FaultableCosineVoltage 按其 MSL 对应元件连接到系统中。faultMode=Normal 或 severity=0 表示名义行为；选择其他 faultMode，并设置 severity、faultStartTime、faultEndTime 和 transitionTime 后仿真故障响应。</p>
