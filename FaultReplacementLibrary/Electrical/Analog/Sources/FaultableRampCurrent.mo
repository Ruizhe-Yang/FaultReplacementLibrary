...
model FaultableRampCurrent "RampCurrent 独立故障增强模型"
  ...
  parameter Modelica.Units.SI.Current I(start=1) "Height of ramp";
...
equation
  ...
  nominalOutput = offset + (if time < startTime then 0 else if time < startTime+duration then I*(time-startTime)/duration else I);
  i = effectiveOutput;
  annotation (
...
<li><em> 1998   </em>
       by Christoph Clauss<br> initially implemented<br>
        ...
        info="<html><p>用法：将 FaultableRampCurrent 按其 MSL 对应元件连接到系统中。faultMode=Normal 或 severity=0 表示名义行为；选择其他 faultMode，并设置 severity、faultStartTime、faultEndTime 和 transitionTime 后仿真故障响应。</p>
