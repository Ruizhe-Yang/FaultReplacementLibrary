...
model FaultableCCC "Linear current-controlled current source"
...
equation
  ...
  i2 = i1*gain_effective;
  v1 = 0;
  annotation (defaultComponentName="ccc",
    Documentation(info="<html><p>用法：将 FaultableCCC 按其 MSL 对应元件连接到系统中。faultMode=Normal 或 severity=0 表示名义行为；选择其他 faultMode，并设置 severity、faultStartTime、faultEndTime 和 transitionTime 后仿真故障响应。</p>
