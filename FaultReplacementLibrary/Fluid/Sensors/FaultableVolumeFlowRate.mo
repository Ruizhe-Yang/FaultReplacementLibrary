...
model FaultableVolumeFlowRate "VolumeFlowRate fluid flow sensor with local signal faults"
  ...
  Modelica.Blocks.Interfaces.RealOutput V_flow(unit="m3/s") annotation(Placement(transformation(origin={0,110},extent={{10,-10},{-10,10}},rotation=270)));
  ...
  Real measuredActual;
  Modelica.Fluid.Sensors.VolumeFlowRate nominal(redeclare package Medium=Medium);
equation
  ...
  measuredActual=nominal.V_flow;
  V_flow=if faultMode==FaultMode.Bias then measuredActual+faultActivation*biasFault elseif faultMode==FaultMode.GainError then measuredActual*(1+faultActivation*(gainFault-1)) elseif faultMode==FaultMode.NoiseIncrease then measuredActual+faultActivation*noiseAmplitude*sin(2*Modelica.Constants.pi*noiseFrequency*time) elseif faultMode==FaultMode.Stuck then measuredActual+faultActivation*(stuckValue-measuredActual) elseif faultMode==FaultMode.Dropout then measuredActual*(1-faultActivation) elseif faultMode==FaultMode.Saturation then measuredActual+faultActivation*(min(saturationLimit,max(-saturationLimit,measuredActual))-measuredActual) else measuredActual;
  annotation(
    Documentation(info="<html><p>用法：将 FaultableVolumeFlowRate 按其 MSL 对应元件连接到系统中。faultMode=Normal 或 severity=0 表示名义行为；选择其他 faultMode，并设置 severity、faultStartTime、faultEndTime 和 transitionTime 后仿真故障响应。</p></html>"),
    Icon(graphics={Ellipse(extent={{-70,70},{70,-70}},lineColor={255,0,0}),Text(extent={{55,90},{90,55}},textString="F",textColor={255,0,0})}));
end FaultableVolumeFlowRate;
